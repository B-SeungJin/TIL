...
## 02.07
* 포인터와 배열, 함수와 관련한 내용들을 배운다.
  - 포인터 배열(포인터 변수로 이루어진 배열), 참조에 의한 호출을 기억해놓자.
## 02.08
* 다차원 배열, 포인터의 포인터에 대하여 정리하였다.
  - int arr[세로][가로]에서 세로길이는 생략 가능. (sizeof(arr)/sizeof(int))는 배열의 길이.
## 02.10
* 2차원 배열이름의 포인터형에 대하여 정리하였다.
  - 포인터 배열과 배열 포인터를 잘 구분하자. 2차원 배열에서는 가로의 길이가 중요. 세로 길이는 함수의 인자로 전달하자.
* 함수 포인터와 void 포인터에 대해 익혔다.
  - void 포인터는 어떠한 주소값도 저장 가능하지만 읽고 쓰기가 불가.
  - main 함수로의 인자전달에 대해 기억하자.
## 02.11
* 문자, 문자열 입출력 함수를 알아보았다.
  - 문자열 입력 함수의 특징(1. 개행문자까지 배열에 저장후 널문자를 붙임-gets와 다름 2. 중간에 공백 삽입 가능) 기억하자.
## 02.13
* 입출력 버퍼의 특징, fflush함수 등의 문자열 관련 함수를 정리하였다.
  - str[strlen(str) -1] = '\0(널문자)' 를 통해 fgets의 개행문자를 제거 가능.
## 02.14
* 구조체에 대해 공부하고 이와 배열, 포인터에 대해 정리하였다.
  - ->연산자(자료구조에 활용)에 익숙해지자.
## 02.15
* typedef선언, 공용체, 열거형의 개념들을 익혔다.
  - 맴버로 배열 선언시, 배열까지도 통째로 복사 가능. 구조체와 공용체의 특징 잘 파악하기.
## 02.17
* 파일(wt-출력, rt-입력)과 스트림에 대해 알아보았다.
  - fclose함수 대신 fflush함수를 통해 출력버퍼만 비울 수 있음.
## 02.19
* 파일 입출력 함수(FILE 구조체의 포인터를 인자로 전달), 바이너리 데이터의 입출력(fread, fwrite), 파일 위치 지시자에 대하여 정리하였다.
## 02.21
* 힙영역(접근-포인터)의 동적할당에 대하여 공부하였다.
  - malloc함수의 형변환 과정, free함수 호출을 통한 메모리 누출 방지하기.
## 02.22
* 선행처리기와 매크로에 대하여 정리하였다.
  - #define ABC(x) ((x)*(x)) 와 같은 형태 사용하기. 작은 크기, 호출의 빈도수가 높은 함수를 매크로로 사용.
  - 문자열내의 매개변수 치환은 #연산자 사용.
## 02.24
* extern선언, #include지시자, 상대경로 지정등에 대해 익힌 후 정리하였다.
  - 모든 헤더파일은 #ifndef~#endif로 감싸는 것이 안전.
## 02.28
* '코딩도장'에서 핵심정리 파트를 통해 복습을 하였다.
## 03.01
* 개강을 맞이해 일정표를 만들었지만 많이 아쉬웠다.
  - toDoList 제작 후 다시 수정해보기.
* forEach, filter 문법 사용을 위해 NodeList 타입을 Array 로 바꾸었다.
  - 스프레드 연산자를 통해 변환.
* 다양한 문법을 사용해보았다.
  - 화살표 문법 사용, 내가 지정한 스코프 안에서만 사용가능한 let 변수 설정.
